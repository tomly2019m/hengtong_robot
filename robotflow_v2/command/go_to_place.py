...
#     time.sleep(1)
#     print(robot_id)
#     print(robot_target_position)

    # 临时性配置，207点使用dock函数
    if robot_target_position == '207' or robot_target_position == 'R207':
        end_wait(robot_id)
        if robot_resource_position == '' or robot_resource_position_ref == 0:
            (task_status,position) = go_to_place_and_dock_wait(robot_id, robot_target_position,200)
        else:
            (task_status,position) = go_to_place_and_dock_wait(robot_id, robot_target_position,200, robot_resource_position, robot_resource_position_ref)
    else:
        end_wait(robot_id)
        if robot_resource_position == '' or robot_resource_position_ref == 0:
            (task_status,position) =  go_to_place_and_wait(robot_id, robot_target_position)
        else:
            (task_status,position) =  go_to_place_and_wait(robot_id, robot_target_position, robot_resource_position, robot_resource_position_ref)
